...
#ifdef UTF8
  Kiloath_greeting_u((DiplomatStringView){.data = name, .len = strlen(name)},
                     &out);
#else
  Kiloath_greeting((DiplomatStringView){.data = name, .len = strlen(name)},
                   &out);
#endif
  output[out.len] = '\0';
  printf("%s\n", output);
...
#ifdef UTF8
  Kiloath2 *kiloath =
      Kiloath2_new_u((DiplomatStringView){.data = name, .len = strlen(name)});
#else
  Kiloath2 *kiloath =
      Kiloath2_new((DiplomatStringView){.data = name, .len = strlen(name)});
#endif
